...
public abstract class MRA extends Agent {
    ...
    public String executeRemoteSkill(MRAInfo mraInfo, SkillTemplate st) throws SkillExecuteException {
        ...
        String result = ""; //cria uma String chamada result vazia
        
        Execute ex = new Execute(); //instancia um objeto da Classe Execute que é a ação responsável pela execução da skill do agente
        ex.setMRAInfo(mraInfo); //seta o mraInfo recebido como argumento no método
        ex.setSkillTemplate(st); // seta SkillTemplate recebido como argumento no método 
        Action act = new Action(getAID(), ex);
        ACLMessage request = new ACLMessage(ACLMessage.REQUEST); //utiliza o protocolo REQUEST para solicitar um receptor executar determinada tarefa
        request.setSender(this.getAID()); // o agente remetente escolhido 
        request.addReceiver(new AID(mraInfo.getAID(), false)); //receptor da mensagem 
        request.setLanguage(new SLCodec().getName()); //seta a linguagem
        request.setOntology(EPSOntology.EPSONTOLOGYNAME); //seta a ontologia da arquitetura
        try {
            ...
            ACLMessage msg = this.blockingReceive(mt, 1000); //modificado 2000 para 1000
            if (msg == null) {
     ...
     * de várias redes de contratos ao mesmo tempo, seguindo o contract-net-protocol do FIPA 
    ...
    protected void addResponderBehaviour() {
        ...
        MessageTemplate template = MessageTemplate.and(
            MessageTemplate.MatchProtocol("fipa-contract-net"), //protocolo interessado
            MessageTemplate.MatchPerformative(ACLMessage.CFP) //ato comunicativo interessado
        );
        ...
        addBehaviour(new CyclicBehaviour(this) {
            ...
            public void action() {
                ...
                if (cfp != null) {
                    myAgent.addBehaviour(new SSContractNetResponder(myAgent, cfp){ //comportamento de pariticpante
                        @Override
                ...
                if ((Util.fromSkill(sk)).equals(requestedSkill)) { 
                    ...
                    try {
                        ...
                        if("failed".equals(sk.getResult())) {
                            ...
                            reply.setContent("true"); //modificação
                        }
